...
export async function fetchAuthorBySlug(slug) {
  try {
    // OPTIMIZED: Query only author column to reduce data transfer
    // Note: This still fetches all podcasts but only the author column
    // For better optimization, consider creating a materialized view or using RPC
    const response = await fetch(
      `${SUPABASE_URL}/rest/v1/podcasts?select=author&is_private=eq.false`,
      {
        headers: {
          'Accept': 'application/json',
          'Content-Type': 'application/json',
...
export async function fetchAllAuthors() {
  try {
    // OPTIMIZED: Query only author column and filter private podcasts at DB level
    const response = await fetch(
      `${SUPABASE_URL}/rest/v1/podcasts?select=author&is_private=eq.false`,
      {
        headers: {
...
export async function fetchRelatedPodcastsByGenre(currentPodcastId, genres, limit = 6) {
  try {
    ...
    // OPTIMIZED: Query podcasts filtered by genre at database level
    // Note: PostgREST JSONB queries are complex, so we fetch and filter client-side
    // For better optimization, consider creating a database function or materialized view
    const response = await fetch(
      `${SUPABASE_URL}/rest/v1/podcasts?select=id,feed_url,title,author,image_url,description,genre,is_private&id=neq.${currentPodcastId}&is_private=eq.false&limit=${limit * 3}`,
      {
        headers: {
          'Accept': 'application/json',
          'Content-Type': 'application/json',
    ...
    if (!response.ok) {
      throw new Error('Failed to fetch related podcasts');
    }

    const podcasts = await response.json();
    
    // Filter by genre match (client-side filtering for JSONB arrays)
...
export async function fetchPodcastsByGenre(genre, limit = 100) {
  try {
    ...
    // OPTIMIZED: Query only public podcasts, filter by genre client-side
    // Note: PostgREST doesn't easily support JSONB array contains queries
    // For better optimization, consider creating a database function or materialized view
    const response = await fetch(
      `${SUPABASE_URL}/rest/v1/podcasts?select=id,feed_url,title,author,image_url,description,genre,is_private&is_private=eq.false&limit=${limit * 2}`,
      {
        headers: {
          'Accept': 'application/json',
          'Content-Type': 'application/json',
    ...
    if (!response.ok) {
      throw new Error('Failed to fetch podcasts by genre');
    }
...
export async function fetchAllGenres() {
  try {
    // OPTIMIZED: Query only genre column and filter private at DB level
    const response = await fetch(
      `${SUPABASE_URL}/rest/v1/podcasts?select=genre&is_private=eq.false`,
      {
        headers: {
          'Accept': 'application/json',
          'Content-Type': 'application/json',
